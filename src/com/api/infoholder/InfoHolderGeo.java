package com.api.infoholder;

import java.io.FileWriter;
import java.io.IOException;
import java.util.HashMap;

import org.apache.commons.lang3.ArrayUtils;

import com.api.geocode.PrefixReplacements;
import com.api.utils.FileUtils;
import com.opencsv.CSVWriter;

public class InfoHolderGeo {

	private String type = null;
	private String oFileType = null;
	private String inStreet;
	private String crStreet;
	private String houseNumber;
	private String street;
	private String borough;
	private String x;
	private String y;
	private boolean isSet;
	private String sourceString;
	private String block;
	private String lot;
	private String[] original;
	private String[] keys;
	private HashMap<String, String> outputValues;
	
	private PrefixReplacements prefixReplacements;
	
	
	public String[] getOriginal() {
		return original;
	}

	public void setOriginal(String[] original) {
		this.original = original;
	}

	public String[] getKeys() {
		return keys;
	}

	public void setKeys(String[] keys) {
		this.keys = keys;
	}

	public boolean putInMap(String key, String value) {
		if (this.outputValues == null) {
			this.outputValues = new HashMap<String, String>();
		}
		if (this.outputValues.containsKey(key)) {
			return false;
		} else {
			this.outputValues.put(key, value);
			return true;
		}
	}

	
	public String getHouseNumber() {
		return houseNumber;
	}

	public void setHouseNumber(String houseNumber) {
		this.houseNumber = removeUnwantedChars(houseNumber);
	}

	public String getStreet() {
		return street;
	}

	public void setInStreet(String inStreet) {
		inStreet = removeUnwantedChars(inStreet);
		String temp = prefixReplacements.replace(inStreet.toUpperCase());
		if (temp!=null){
			this.inStreet = temp;
		}else{
			this.inStreet = inStreet;
		}
	}
	
	public void setCrStreet(String crStreet) {
		crStreet = removeUnwantedChars(crStreet);
		String temp = prefixReplacements.replace(crStreet.toUpperCase());
		if (temp!=null){
			this.crStreet = temp;
		}else{
			this.crStreet = crStreet;
		}
	}

	public void setStreet(String street) {
		street = removeUnwantedChars(street);
		String temp = prefixReplacements.replace(street.toUpperCase());
		if (temp!=null){
			this.street = temp;
		}else{
			this.street = street;
		}
	}

	public String getBorough() {
		return borough;
	}

	public void setBorough(String borough) {
		this.borough = removeUnwantedChars(borough);
	}

	public String getX() {
		return x;
	}

	public void setX(String x) {
		this.x = x;
	}

	public String getY() {
		return y;
	}

	public void setY(String y) {
		this.y = y;
	}

	public HashMap<String, String> getSetValues() {
		HashMap<String, String> toReturn = new HashMap<String, String>();
		if (houseNumber != null && houseNumber.trim().length() > 0) {
			toReturn.put("houseNumber", houseNumber);
		}
		if (street != null && street.trim().length() > 0) {
			toReturn.put("street", street);
		}
		if (borough != null && borough.trim().length() > 0) {
			toReturn.put("borough", borough);
		}
		if (inStreet != null && inStreet.trim().length() > 0) {
			toReturn.put("inStreet", inStreet);
		}
		if (crStreet != null && crStreet.trim().length() > 0) {
			toReturn.put("crStreet", crStreet);
		}
		return toReturn;
	}

	public InfoHolderGeo() {
		prefixReplacements = new PrefixReplacements();
		return;
	}

	public String getCrStreet() {
		return crStreet;
	}

	public String getInStreet() {
		return inStreet;
	}

	public String getType() {
		return type;
	}

	public void setType(String type) {
		this.type = removeUnwantedChars(type);
	}

	public String getoFileType() {
		return oFileType;
	}

	public void setoFileType(String oFileType) {
		this.oFileType = removeUnwantedChars(oFileType);
	}

	private String removeUnwantedChars(String input){
		return input.replaceAll("[^A-Za-z0-9% ]", "");
	}

	public boolean isSet() {
		return isSet;
	}

	public void setSet(boolean isSet) {
		this.isSet = isSet;
	}

	public String getSourceString() {
		return sourceString;
	}

	public void setSourceString(String sourceString) {
		this.sourceString = sourceString;
	}

	public String getBlock() {
		return block;
	}

	public void setBlock(String block) {
		this.block = block;
	}

	public String getLot() {
		return lot;
	}

	public void setLot(String lot) {
		this.lot = lot;
	}
	
	public void printCSV(FileWriter writer) {
		try {
	//		writer.write("\"");
			for (String value : original) {
				writer.write(FileUtils.formatForCSV(value) + ",");
			}
			if (this.outputValues != null) {
				for (String key : this.keys) {
					writer.write(FileUtils.formatForCSV(this.outputValues.get(key))
							+ ",");
				}
			} else{
				for (String key : this.keys) {
					writer.write(FileUtils.formatForCSV("null")
							+ ",");
				}
			}
			writer.write("\r\n");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}
	public void writeCSV(CSVWriter writer) {
		String[] output = new String[this.keys.length];
		int i=0;
		for (String key : this.keys) {
			if (this.outputValues.containsKey(key)){
				output[i] = this.outputValues.get(key);
			} else{
				output[i] = null;
			}
			i++;
		}
		String[] toWrite = ArrayUtils.addAll(original,output);
		writer.writeNext(toWrite);
	}
	
/*	public void printCSV(FileWriter writer) {
		try {
			writer.write("\"" + getRowid() + "\",\""
					+ formatForCSV(getJournal()) + "\",\""
					+ formatForCSV(getArticle()) + "\",\""
					+ formatForCSV(getCitations()) + "\",\""
					+ formatForCSV(getUrl()) + "\",\""
					+ getAvailable() + "\",\"" + getDateCapturedFormatted()
					+ "\",\"" + getHrURL() + "\"\r\n");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	}*/
}
